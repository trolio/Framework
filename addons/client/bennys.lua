...
function OpenLSMenu(elems, menuname, menutitle, parent)
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu) -- on validate
			...
			local vehicle = GetVehiclePedIsIn(GetPlayerPed(-1), false)			
			local found = false
			for k,v in pairs(Config.BennysMenus) do
				...
				if data.value == 'primaryRespray' or data.value == 'secondaryRespray' or data.value == 'pearlescentRespray' or data.value == 'modFrontWheelsColor' then
					for i=1, #Config.BennyColors, 1 do
						if data.value == 'primaryRespray' then
							table.insert(elements, {label = Config.BennysColors[i].label, value = 'color1', color = Config.BennysColors[i].value})
						elseif data.value == 'secondaryRespray' then
							table.insert(elements, {label = Config.BennysColors[i].label, value = 'color2', color = Config.BennysColors[i].value})
						elseif data.value == 'pearlescentRespray' then
							table.insert(elements, {label = Config.BennysColors[i].label, value = 'pearlescentColor', color = Config.BennysColors[i].value})						
						elseif data.value == 'modFrontWheelsColor' then
							table.insert(elements, {label = Config.BennysColors[i].label, value = 'wheelColor', color = Config.BennysColors[i].value})
						end
					end
				else
...
Citizen.CreateThread(function()
	...
	for k,v in pairs(Config.BennysZones)do
...
Citizen.CreateThread(function()
	while true do
		...
		if IsPedInAnyVehicle(playerPed, false) then
			...
			local zone 		  = nil
			local lastZone    = nil
			if (PlayerData.job ~= nil and PlayerData.job.name == 'mecano') or Config.BennysIsMecanoJobOnly == false then
				for k,v in pairs(Config.BennysZones) do
